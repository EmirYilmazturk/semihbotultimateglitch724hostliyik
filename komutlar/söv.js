...
exports.run = async (client, msg, args) => {
   ...
   description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
    ...
    if(member.id === "464471005444833280")return msg.channel.send({embed: {
   color: Math.floor(ayarlar.renk),
   description: (':no_entry_sign:Yapımcıma Haa sakın. Görmiyim bidaha.')
  }})
    if(member.id === "410422439823015968")return msg.channel.send({embed: {
   color: Math.floor(ayarlar.renk),
